...
오늘 요일에 해당하는 기사님 차량의 활성 시간대 목록을 반환합니다.

- Query parameters:
  - `date` optional, `YYYY-MM-DD`, 개발 확인용 날짜 지정
...
현재 관리자 API는 관리자 인증/권한 기반, 안전한 조회 API, 원생/차량/시간표 관리, 출결 기록 조회/Excel export, Excel import preview, SMS 로그 조회를 제공합니다.
...
관리자가 원생과 보호자 연락처를 추가합니다. 보호자 이름과 연락처는 선택 입력이며, 연락처는 `000-0000-0000` 형식의 숫자 11자리만 허용하고 숫자만 정규화해 저장합니다. 기사님 전달 메모는 선택 입력이며 최대 20자입니다.
...
- Required auth: `admin`
- Query parameters:
  - `dayOfWeek` optional: `monday` | `tuesday` | `wednesday` | `thursday` | `friday` | `saturday` | `sunday`
  - `vehicleId` optional
...
```json
{
  ...
  "data": {
    "schedules": [
      {
        "scheduleId": "schedule_monday_1330",
        "vehicleId": "vehicle_1",
        "vehicleName": "1호차",
        "dayOfWeek": "monday",
        "name": "등원",
...
### GET `/api/admin/attendance-records/export`

관리자 출결 기록을 Excel 파일로 다운로드합니다. 학부모 연락처는 포함하지 않습니다.
...
SMS 로그를 조회합니다. 수신번호는 마스킹 값만 반환합니다.

- Required auth: `admin`
- Query parameters:
  - `date`: 선택, `YYYY-MM-DD`

발신번호, API Key, API Secret은 환경변수로만 설정합니다.

## 13. Excel API 개요

Excel은 DB의 원본 데이터를 가져오거나 내보내는 보조 수단입니다. Excel 파일 자체가 원본 데이터가 되지는 않습니다.

### POST `/api/admin/excel/import/preview`

학생/차량/요일/시간표 배정용 Excel 파일을 미리 파싱하고 검증합니다. 이 API는 DB에 저장하지 않습니다.
...
응답에는 최대 100개 preview row와 검증 요약을 반환합니다.
