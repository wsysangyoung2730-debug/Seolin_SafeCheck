...
이 문서는 API 경로, 공통 응답 형식, 개발용 인증 방식, 기사님용 일정/원생/출결 저장 API, 관리자 read-only API 기반을 설명합니다. 현재 로컬 백엔드는 PostgreSQL schema/seed와 repository 계층을 사용합니다. **운영용 인증, SMS 발송, Excel 처리, 관리자 대시보드 UI 구현은 아직 포함되지 않습니다.**
...
- 현재 admin API는 관리자 인증/권한 기반과 read-only 조회 API만 제공합니다.
...
현재 관리자 API는 관리자 인증/권한 기반과 안전한 read-only 조회 API만 제공합니다. 관리자 대시보드 UI, CRUD, SMS 발송, Excel import/export는 구현하지 않습니다.
...
관리자 원생 목록 화면의 기반이 되는 read-only 원생 목록을 반환합니다. 학부모 연락처 원문은 반환하지 않고 등록 여부만 반환합니다.
...
관리자 차량 목록 화면의 기반이 되는 read-only 차량 목록을 반환합니다.
...
관리자 운행 시간대 목록 화면의 기반이 되는 read-only 일정 목록을 반환합니다.
...
관리자 출결 기록 조회 화면의 기반이 되는 최근 출결 기록을 반환합니다. 결과는 최근 100건으로 제한합니다.
...
- SMS 발송, SMS 로그 생성, Excel import/export, 관리자 CRUD API, 관리자 대시보드 UI는 아직 구현하지 않았습니다.
- 현재 구현된 관리자 API는 개발용 admin 로그인, 권한 확인, read-only overview/students/vehicles/schedules/attendance-records 조회입니다.
