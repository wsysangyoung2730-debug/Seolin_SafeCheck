...
이 문서는 API 경로, 공통 응답 형식, 개발용 인증 방식, 기사님용 일정/원생/출결 저장 API, 관리자 API 기반과 원생 관리 MVP API를 설명합니다. 현재 로컬 백엔드는 PostgreSQL schema/seed와 repository 계층을 사용합니다. **운영용 인증, SMS 발송, Excel 처리, 차량/시간표 관리 UI 구현은 아직 포함되지 않습니다.**
...
- 현재 admin API는 관리자 인증/권한 기반, read-only 조회 API, 원생 관리 MVP write API를 제공합니다.
...
- SMS 발송, SMS 로그 생성, Excel import/export, 차량/시간표 CRUD API는 아직 구현하지 않았습니다.
- 현재 구현된 관리자 API는 개발용 admin 로그인, 권한 확인, overview/students/vehicles/schedules/attendance-records 조회, 원생 생성/수정/비활성화입니다.
