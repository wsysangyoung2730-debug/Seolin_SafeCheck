...
SMS는 출결 저장 이후 mock provider를 통해서만 처리합니다. 실제 SMS는 발송하지 않습니다.

### GET `/api/admin/sms-logs`

mock SMS 로그를 조회합니다. 수신번호는 마스킹 값만 반환합니다.

- Required auth: `admin`
- Query parameters:
  - `date`: 선택, `YYYY-MM-DD`

실제 SMS provider, 발신번호, 과금 방식, 실패 재시도 정책은 추후 확정 필요입니다.
...
- SMS는 mock provider와 로그 저장만 구현되어 있으며 실제 발송은 하지 않습니다.
