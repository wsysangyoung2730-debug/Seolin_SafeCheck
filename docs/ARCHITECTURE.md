...
- 현재 정적 admin MVP는 `/admin/login/`, `/admin/home/`, `/admin/students/`에서 관리자 로그인과 원생 관리를 먼저 제공합니다.
...
현재 구현된 admin API는 인증/권한 기반, read-only 조회 API, 원생 생성/수정/비활성화 API입니다. 차량, 시간표 CRUD, SMS, Excel 처리는 후속 작업에서 구현합니다.
