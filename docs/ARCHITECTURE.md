...
- 현재 정적 admin MVP는 `/admin/login/`, `/admin/home/`, `/admin/students/`, `/admin/vehicles/`, `/admin/schedules/`, `/admin/attendance/`에서 관리자 로그인, 원생/차량/시간표 관리, 시간표별 원생 배정, 출결 기록 조회를 제공합니다.
...
현재 구현된 admin API는 인증/권한 기반, 조회 API, 원생/차량/시간표 생성/수정/비활성화 API, 시간표별 원생 배정 API, 출결 기록 조회 API입니다. SMS, Excel 처리는 후속 작업에서 구현합니다.
